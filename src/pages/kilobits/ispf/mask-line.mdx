...
You can remove the mask using one of these methods:
1. Use the [Delete (D)](/kilobits/ispf/delete-line/) command on the `=MASK>` line, followed by <kbd>Enter</kbd>.
2. Typing `RESET` or `END` into the primary command line, followed by <kbd>Enter</kbd>.
3. Ending your edit session by pressing <kbd>F3</kbd>. 
