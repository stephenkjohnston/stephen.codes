...
The overlay line command is used with the [`COPY`](/kilobits/ispf/copy-line/) or `MOVE` line commands. It allows you to copy or move non-blank characters from one line to another line using `O` (single line) or from multiple lines using `OO` (block of lines). If a non-blank character is encountered in the destination line(s), it will skip over it, leaving it unchanged.
